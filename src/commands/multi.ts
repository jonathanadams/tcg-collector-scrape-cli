...
export const multiCommand = new Command("multi")
  .description(
    "Select sets from TCGCollector and scrape each in its own folder"
  )
  .requiredOption("-o, --output <path>", "Output root folder to save sets")
  .option(
    "-c, --concurrency <n>",
    "How many sets to scrape at the same time",
    "3"
  )
  .option(
    "-f, --force",
  ...
  .action(
    async ({
      ...
      await page.goto(
        "https://www.tcgcollector.com/sets/intl?setMode=regularCardVariants&releaseDateOrder=newToOld&displayAs=list",
      ...
      const seriesData = await page.$$eval(".set-list", (seriesEls) => {
        return seriesEls.map((el) => {
          const title =
            el.querySelector("h2.set-list-title")?.textContent?.trim() || "";
          const setLinks = Array.from(
            el.querySelectorAll("a.set-list-item-set-name")
          ).map((a) => ({
            name: a.textContent?.trim() || "",
            href: (a as HTMLAnchorElement).href + "?displayAs=list",
          }));
      ...
      const { chosenSeries } = await inquirer.prompt<{
        chosenSeries: (typeof seriesData)[number][];
      }>([
        {
          type: "checkbox",
          name: "chosenSeries",
      ...
      type SetWithSeries = (typeof seriesData)[number]["sets"][number] & {
        seriesName: string;
      };

      const allSelectedSets: SetWithSeries[] = [];

      for (const series of chosenSeries) {
        const { selectedSets } = await inquirer.prompt<{
          selectedSets: (typeof seriesData)[number]["sets"][number][];
        }>([
          {
            type: "checkbox",
            name: "selectedSets",
      ...
      await runConcurrent(
        ...
        async (set, index) => {
          ...
          if (fs.existsSync(dir) && force) {
            console.log(
              chalk.redBright(
                `⚠️  Removing old ${seriesName}/${setName} before re-scraping`
              )
            );
            ...
            console.log(
              chalk.yellow(
                `⏭️  Skipping ${seriesName}/${setName} (already scraped)`
              )
            );
            await page.close();
            return;
          ...
          try {
            ...
            console.error(
              chalk.red(`❌ Failed ${set.name}: ${(err as Error).message}`)
            );
          } finally {
