...
export function saveCsv(data, outputFile) {
    const { cards } = data;
    ...
    const rows = cards.map((card) => {
        ...
        const variantFlags = variants.map((variant) => card.variants.includes(variant) ? "true" : "false");
        return [...base, ...variantFlags];
    });
    // Combine into CSV string
    const csvString = [header, ...rows]
        .map((row) => row
        .map((cell) => typeof cell === "string" && cell.includes(",")
        ? `"${cell.replace(/"/g, '""')}"`
        : cell)
        .join(","))
        .join("\n");
